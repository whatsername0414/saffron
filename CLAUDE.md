...
11. Welcome onboarding — 3-slide first-launch screen, translated pixel-for-pixel from the Claude Design `templates/welcome-onboarding/WelcomeOnboarding.dc.html` template:
   ...
   - First-launch gate — `OnboardingRepository` (`:core:database`, SharedPreferences-backed) exposes `hasCompletedOnboarding()` / `setOnboardingCompleted()`. `SaffronApp` reads it via `koinInject<OnboardingRepository>()` to pick the real `NavHost` `startDestination` (`Screen.Welcome` vs `Screen.Home`). Both "Get started" and "Skip" call `WelcomeViewModel.completeOnboarding()` then navigate to Home with `popUpTo(Screen.Welcome) { inclusive = true }` — Welcome never reappears once completed, and it carries no bottom nav bar (excluded from `tabRoutes`). `WelcomeViewModel` has no `skip()` slide-jump method; Skip and Get started share the same completion path.
...
13. App icon & splash screen — brand launcher icon and matching Android 12+ splash, built from the Saffron thread mark:
   - `app/src/main/res/drawable/ic_launcher_background.xml` (flat `#C8860A` fill) + `ic_launcher_foreground.xml` (white thread mark — center bloom, two filaments, stem — scaled inside a `<group>` transform to sit inside the adaptive-icon safe zone). Referenced by the existing `mipmap-anydpi-v26/ic_launcher.xml` / `ic_launcher_round.xml` adaptive-icon definitions (background/foreground/monochrome).
   - Pre-Android-12 fallback — the old default per-density `mipmap-{h,m,xh,xxh,xxxh}dpi` webp folders (stock Android Studio green-robot icon) were deleted and replaced with a single density-independent `mipmap/ic_launcher.xml` / `ic_launcher_round.xml` (the full rounded-square mark, background baked in — same geometry as `app/src/main/res/drawable/ic_saffron_logo.xml`). Vector drawables render natively from API 21+, so no per-density PNGs are needed at minSdk 24.
   - Splash screen — `androidx.core:core-splashscreen` (`libs.androidx.core.splashscreen`) added to `:app`. `Theme.Saffron.Starting` (`values/themes.xml`, parent `Theme.SplashScreen`) sets `windowSplashScreenBackground` to `@color/saffron` and `windowSplashScreenAnimatedIcon` to the same `ic_launcher_foreground` mark, then hands off via `postSplashScreenTheme` to `Theme.Saffron`. `MainActivity`'s manifest theme is `Theme.Saffron.Starting`; `MainActivity.onCreate` calls `installSplashScreen()` before `super.onCreate()`.
   - The system splash screen API has no text slot (background color + masked center icon + optional bottom branding image only), so the splash is icon-only — consistent with the Welcome onboarding screen carrying the wordmark brand moment instead.
